...
def test_gzip_writer_handles_partial_filesystem_writes():
    class ShortSink(io.BytesIO):
        def write(self, body):
            return super().write(bytes(body[:max(1, len(body)//3)]))
    sink = ShortSink()
    complete = normalizer.CompleteWriter(sink)
    body = b'contract,amount\n20200101,10000\n' * 1000
    with gzip.GzipFile(fileobj=complete, mode='wb', mtime=0) as compressed:
        compressed.write(body)
    assert gzip.decompress(sink.getvalue()) == body


def test_imported_scope_and_observation_are_not_invented_from_filename(tmp_path):
    path, base = source(tmp_path)
    entry = dict(base['metadata'])
    entry.pop('downloaded_at')
    entry['query'] = {'kind':'sale','start':'2020-01-01','end':'2020-12-31'}
    entry['raw_sha256'] = entry['sha256']
    entry['file'] = 'sale.csv.gz'
    entry['imported_at'] = '2026-09-09T08:51:45+00:00'
    entry['metadata_rows'] = [[v] for v in ['계약일자 : 2020-01-01 ~ 2020-12-31',
        '실거래구분 : 아파트(매매)','주소구분 : 지번주소','시도 : 경기도',
        '시군구 : 전체','읍면동 : 전체','면적 : 전체','금액선택 : 전체']]
    result = normalizer.source_metadata(path,entry)
    assert result['observation_basis']=='imported_at'
    assert 'downloaded_at' not in result['metadata']
    entry['metadata_rows'][-1] = ['금액선택 : 1억 이하']
    with pytest.raises(ValueError,match='scope'):
        normalizer.source_metadata(path,entry)
