...
def source_metadata(meta_path, entry=None):
    ...
    if entry is not None:
        ...
        meta['request'] = meta['request_fields']
        meta['sha256'] = meta.get('raw_sha256')
        meta['downloaded_at'] = meta.get('downloaded_at', meta.get('count_checked_at'))
    ...
    observed = datetime.fromisoformat(meta['downloaded_at'])
...
class PartitionWriter:
    ...
    def write(self, key, record, *, dashboard=False):
        ...
        if state is None:
            ...
            stack = ExitStack()
            stream = gzip_text(stack, self.stage / name, 'utf-8-sig' if dashboard else 'utf-8')
            writer = csv.DictWriter(stream, fieldnames=DASHBOARD_FIELDNAMES) if dashboard else stream
            if dashboard:
                writer.writeheader()
            state = self.active[key] = {'stack': stack, 'writer': writer, 'rows': 0, 'name': name}
