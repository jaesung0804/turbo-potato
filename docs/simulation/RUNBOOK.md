...
python -m pytest -q tests/test_estate_simulation.py tests/test_estate_simulation_storage.py tests/test_estate_simulation_comparison.py
```

`demo`는 합성자료의 보유·갈아타기·최저가보유·현금 및 무작위보유 3회(seed 7·23·41), 총 7개 시나리오와 동일 조건 비교를 요약 출력한다. 세 seed는 산술 테스트 예시이며 실제 실험에 사전 등록된 값이 아니다. 네트워크·DB 쓰기·수집·학습·홈페이지 배포를 하지 않는다. 실제 실행 요청은 명시적인 `run` 하위명령이다. 새 자동실행 스케줄이나 홈페이지 트리거는 추가하지 않았다.

## 2026-09-18 비교 실행 인계

무작위 기준선의 정책에는 `strategy=random_hold`, `random_seed=<사전 등록 uint32 정수>`를 명시한다. 다른 전략은 seed를 넣지 않는다. 각 전략/seed는 서로 다른 run ID와 준비 체크포인트를 사용하며, 같은 패널·기간·총현금·예비자금·가격상한·비용·체결 규칙을 유지한다. 실자료에는 전략별로 기존 `BackendSession`과 저장 준비 검증이 모두 필요하다.

실행을 마친 뒤 `compare_replays(results, random_seeds=(...))`에 원래 `replay()` 결과를 전달한다. 이 함수는 추가 실험이나 DB 조회를 시작하지 않는다. CLI 출력의 요약만으로 원래 체크포인트를 대신할 수 없으며, 압축 원장/체크포인트는 기존 DB·OCI 경로에 보존한다. 예산·비용 민감도별로 비교 묶음을 따로 만들고 서로 섞지 않는다.

보고할 것은 전략별 최종 현금·거래비용·매수 체결 여부·미결 계약·미청산 상태, 모든 등록 seed의 분모, 보유 대비 갈아타기 및 각 기준선 대비 차이다. 하나라도 결과가 없으면 누락된 채로 비교를 통과시키지 않는다. 가장 잘 나온 seed만 고르거나 청산 가능한 seed만 평균내지 않는다. 현금 결과가 0%인 미체결 사례도 남긴다.

이번 구현으로 엔진 해시와 정책 형태가 달라졌다. 옛 체크포인트를 새 엔진에 맞춰 고치지 말고, 이전 엔진으로 재개하거나 출처를 보존한 새 실험 ID를 등록한다. 현재까지 실제 역사 실험은 0회이며, 무료 용량 계측·실자료 입력 검수 조건은 계속 적용된다.
...
남은 기능(9월 18일 갱신): 실제 세제/대출·임시거주·점유·인도일 모듈, 사후 최적 상한의 독립 집계, 지역·가격대 대조군, 피처 학습 파이프라인 연결, 실제자료 성능검증. 무작위 대조군과 동일 조건 비교는 합성자료로 검증했다. 현재 가격 모형과 대표 홈페이지 배포는 이 엔진을 import하는 것만으로 변하지 않는다.
