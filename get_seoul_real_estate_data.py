...
# | `CGG_CD`                | 자치구 코드     | 지역 식별              |
# | `CGG_NM`                | 자치구명       | 지역명                |
# | `STDG_CD`               | 법정동 코드     | 동 단위 식별            |
# | `STDG_NM`               | 법정동명       | 동 이름               |
# | `LOTNO_SE`              | 지번 구분 코드   | 대지/산 구분            |
# | `LOTNO_SE_NM`           | 지번 구분명     | 대지, 산 등            |
# | `MNO`                   | 본번         | 주소 구성              |
# | `SNO`                   | 부번         | 주소 구성              |
# | `BLDG_NM`               | 건물명        | 단지명/건물명            |
# | `CTRT_DAY`              | 계약일        | 거래일자               |
# | `THING_AMT`             | 물건금액       | 거래금액, 보통 만원 단위     |
# | `ARCH_AREA`             | 건물면적       | 전용면적 또는 건축물 면적, ㎡  |
# | `LAND_AREA`             | 토지면적/대지권면적 | 대지지분 계산 핵심         |
# | `FLR`                   | 층          | 층수                 |
# | `RGHT_SE`               | 권리구분       | 입주권/분양권 등 권리 유형 가능 |
# | `RTRCN_DAY`             | 해제사유발생일    | 거래취소 여부 확인         |
# | `ARCH_YR`               | 건축년도       | 연식 계산              |
# | `BLDG_USG`              | 건물용도       | 아파트, 연립다세대, 오피스텔 등 |
# | `DCLR_SE`               | 신고구분       | 직거래/중개거래           |
# | `OPBIZ_RESTAGNT_SGG_NM` | 중개사 소재지    | 중개거래 시 중개사 지역      |
