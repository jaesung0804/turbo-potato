...
def build_bundle(summary,model,root,map_data):
    root.mkdir(parents=True,exist_ok=True);catalog=[];identities={};regions=[];periods={};history={}
    for r in summary['regions']:
        ...
        for year,bucket in [('all',r['all']),*r['years'].items()]:
            ...
            for a in bucket['addresses']:
                info={k:v for k,v in a.items() if k not in {'count','metrics'}}
                ...
                idx=identities[identity];rows.append([idx,a['count'],compact(a['metrics'])])
                if year!='all':
    ...
    manifest={'schema_version':1,'generated_at':summary.get('data_through',summary['generated_at']),
        ...
        'filters':summary.get('filters',{}),'model':{k:v for k,v in model.items() if k!='recommendations'},
