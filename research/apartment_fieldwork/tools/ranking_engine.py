...
def rank_one(candidates, constraints, living_weight, investment_weight, *, require_both=True):
    ...
    for candidate in candidates:
        ...
        if reasons:
            ...
            row["provisional"] = not candidate.get("current_listing_verified", False)
            ranked.append(row)
    ...
    for position, row in enumerate(ranked, start=1):
        if previous is not None and math.isclose(row["score"], previous["score"], abs_tol=1e-9):
            ...
            row["rank"] = position
        previous = row
    return {"weights": weights, "ranked": ranked, "held": held}
...
def run(payload):
    ...
    result = {"schema_version": 1, "score_meanings": payload["score_meanings"], "constraints": constraints,
